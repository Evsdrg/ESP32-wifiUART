...
bool requireAuthentication() {
  ...
  if (!server.authenticate(HTTP_AUTH_USERNAME, HTTP_AUTH_PASSWORD)) {
    server.requestAuthentication(BASIC_AUTH, DEVICE_LABEL);
    return false;
  }
#endif
  return true;
...
void handleSetUartSettings() {
  ...
  if (bridge::isTcpClientConnected() || rfc2217_bridge::isClientConnected()) {
    sendJsonError(409, "Cannot change UART settings while a bridge session is active");
    return;
  }
  ...
  if (!uart_port::applySettings(requested, nullptr, 1000)) {
